...
project files needed. Run this from your own terminal to:
  1. Retroactively fix Daily Job Log PDF filenames AND descriptions --
     re-derives each file's real Name/Date/Job from its own text content,
     renames it, and refreshes its .txt sidecar description too.
...
def derive_correct_filename(dbx, folder_path, entry):
    ...
    if answers and has_real_titles:
        ...
        new_job_wo, extracted_text, extra = parse_structured_answers(
